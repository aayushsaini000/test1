...
def get_driver_text(page, driver):
    ...
    if any("unknown" in item.lower() for item in text):
        ...
        driver = [" ".join([text[2], text[3]]), ", ".join([text[1], text[0]])]
...
def check_pdf(page, reader, page_size):
    ...
    pdf_check = reader.readtext(gray, decoder="greedy", detail=0, paragraph=False)

    if any("new jersey" in s.lower() for s in pdf_check):
        return True
    else:
        return False
...
def get_date_ocr(page, reader, page_size):
    ...
    date_ocr = reader.readtext(image, decoder="greedy", detail=0, paragraph=False)
...
def get_driver_one_ocr(page, reader, page_size):
    ...
    driver_one = reader.readtext(
        thresh, decoder="greedy", detail=0, paragraph=True, x_ths=0.95
    )
...
def get_driver_two_ocr(page, reader, page_size):
    ...
    driver_two = reader.readtext(
        thresh, decoder="greedy", detail=0, paragraph=True, x_ths=0.3
    )
...
def get_occupants_ocr(page, reader, page_size, driver_one, driver_two):
    ...
    occupants = reader.readtext(image, decoder="greedy", detail=0, paragraph=True)

    if any("driver 1" in s.lower() for s in occupants):
        occupants_ocr = reader.readtext(
            image, decoder="greedy", detail=0, paragraph=True, y_ths=0.3, x_ths=30
        )
